        ...
        <div className="dashboard-page-bg">
            <div className="dashboard-glass-box">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                    <div>
                        <h2>LEAP — User Administration</h2>
                        <p style={{ color: '#718096', margin: '8px 0 0 0', fontSize: '14px' }}>
                            Logged in as <strong>{currentUser?.email}</strong>
                            {currentUser?.role && <> · <span style={{ textTransform: 'capitalize' }}>{currentUser.role}</span></>}
                        </p>
                    </div>
                ...
                {editingUser && (
                    <div className="edit-form-container">
                        <h3>✏️ Edit User #{editingUser.user_id}</h3>
                        <form onSubmit={handleUpdate}>
                            ...
                            <input className="app-input" type="email" placeholder="Email"       value={editingUser.email       || ''} onChange={e => setEditingUser({ ...editingUser, email:       e.target.value })} required
                                pattern=".*(@gbox\.ncf\.edu\.ph|@ncf\.edu\.ph)$" title="School email required" />
                            ...
                            <div style={{ display: 'flex', gap: '10px' }}>
                                <button type="submit" className="app-button" style={{ margin: 0, width: 'auto' }}>💾 Save</button>
                ...
                {loading ? (
                    ...
                    <>
                        {/* Stats */}
                        <div style={{ background: 'linear-gradient(135deg,rgba(34,197,94,.1),rgba(22,163,74,.1))', padding: '20px', borderRadius: '16px', marginBottom: '30px', display: 'flex', gap: '30px', flexWrap: 'wrap' }}>
                            {[
                                ...
                                <div key={s.label}>
                                    <p style={{ margin: 0, color: '#718096', fontSize: '13px', fontWeight: '600' }}>{s.label}</p>
                            ...
                            <tbody>
                                {users.length === 0 ? (
                                    ...
                                    <tr key={u.user_id}>
                                        ...
                                        <td>{u.email}</td>
                                        <td><span style={roleStyle(u.role)}>{u.role === 'instructor' ? '👨‍🏫' : '👨‍🎓'} {u.role}</span></td>
                                        <td style={{ fontSize: '13px', color: '#718096' }}>
                                            {u.role === 'student'    && <>Year {u.year_level || '—'} · Dept {u.dept_id || '—'}</>}
                                            {u.role === 'instructor' && <>{u.specialization || '—'}</>}
                                        </td>
                                        <td style={{ textAlign: 'center' }}>
                                            <button onClick={() => setEditingUser(u)}>✏️ Edit</button>
                                            <button onClick={() => handleDelete(u.user_id)}>🗑️ Delete</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
